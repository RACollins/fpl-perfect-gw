...
def download_from_github(season, gw):
    df = pd.read_csv(
        "https://raw.githubusercontent.com/vaastav/Fantasy-Premier-League/master/data/{0}/gws/gw{1}.csv".format(
            season, gw
        )
    )
    return df
...
def main():
    ...
    print(
        df.loc[df["total_points"] > 4, :].sort_values(["total_points"], ascending=False)
    )
